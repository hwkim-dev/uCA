...
emits micro-ops (\textit{μops}) into the Dispatcher, which resolves
...
\begin{tikzpicture}[
  ...
  % Arrows
  \draw[arr] (cmd) -- (dec);
  \draw[arr] (dec) -- node[above, font=\sffamily\scriptsize]{μop} (disp);
  \draw[arr, <->] (disp) -- (gs);
...
\begin{figure}[h]
\centering
\begin{bytefield}[bitwidth=1.0em, bitheight=2.3em, boxformatting={\centering\sffamily\scriptsize}]{64}
...
\begin{tikzpicture}[
  ...
  % 5 branches
  \node[branch, right=1.6cm of dec, yshift=2.4cm]  (b0) {GEMV μop};
  \node[branch, right=1.6cm of dec, yshift=1.4cm]  (b1) {GEMM μop};
  \node[branch, right=1.6cm of dec, yshift=0.4cm]  (b2) {MEMCTRL μop};
  \node[branch, right=1.6cm of dec, yshift=-0.6cm] (b3) {MEMSET μop};
  \node[branch, right=1.6cm of dec, yshift=-1.6cm] (b4) {CVO μop};
...
μop types; the dispatcher resolves inter-instruction hazards before
handing off to the back end.}
\label{fig:decode-dispatch}
\end{figure}

\section{μop Decomposition Summary}

\begin{longtable}{|l|p{10cm}|}
\hline
\rowcolor{skytint}
\textbf{μop type} & \textbf{Constituent fields (width)} \\
\hline
\endfirsthead
...
\begin{figure}[h]
\centering
\begin{bytefield}[bitwidth=1.05em, bitheight=2.3em, boxformatting={\centering\sffamily\scriptsize}]{64}
...
\begin{center}
\begin{bytefield}[bitwidth=1.05em, bitheight=2.2em, boxformatting={\centering\sffamily\scriptsize}]{64}
  \bitheader[endianness=big,lsb=0]{0,3,8,14,20,26,43,60,63} \\
  \bitbox{4}{opcode} &
...
\begin{tikzpicture}[
  ...
  \node[blocknode, minimum width=3cm] (in) {Incoming μop};
...
\end{tikzpicture}
\caption{Hazard pipeline. Each μop passes an address-hazard and a
...
\begin{tikzpicture}[
  ...
  \draw[arrfat] ($(c0)+(0,-1.2cm)$) -- node[above, font=\sffamily\scriptsize]{μop + \texttt{fence\_id}} ($(b0)+(0,-1.2cm)$);
...
\begin{itemize}[label={\color{oceanblue}\textbullet}, leftmargin=*, itemsep=0.3ex]
    ...
    \item Fuse softmax: on the same SFU dispatch, chain
          ...
          \texttt{CVO\_SCALE(recip\_scale)} so the pipeline drains
