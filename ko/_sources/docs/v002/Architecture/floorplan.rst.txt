...
- **상하 대칭 슬라이스**: 각 슬라이스에는 32×1 GEMV 코어 2 개 (양
  슬라이스 합 4 개) 와 코어별 Constant Cache, L1 Cache 가 포함됩니다.
  디코딩 단계에서 두 슬라이스가 병렬로 서로 다른 배치 / 헤드 또는
  multi-query 를 처리할 수 있습니다. SFU 는 단일 인스턴스로 두 슬라이스
  사이에 배치되어, 양쪽 GEMV↔SFU 직결 FIFO 의 배선 길이를 대칭으로
  유지합니다.
- **우측 시스톨릭 어레이**: 32×32 GEMM 코어 1 개 (cascade 가 16 행에서
  끊겨 32×16 서브체인 2 개로 나뉨) 가 프리필 단계의 대규모 GEMM 을
  담당합니다.
