...
.. figure:: ../../../../assets/images/Architecture/v002/architecture_v002.png
   ...
   **Figure 2.** pccx v002 의 물리 플로어플랜.
   L2 Cache가 중앙에 배치되어 상하 대칭으로 배치된 GEMV·SFU 뱅크와
   양측에 위치한 시스톨릭 어레이에 액티베이션을 공급한다.

1. 대칭형 배치 전략
===================

pccx v002의 핵심 물리적 특징은 **중앙 공유 L2 캐시 + 대칭 컴퓨트 슬라이스**
구조이다.

- **중앙 L2 캐시**: 플로어플랜의 기하학적 중심에 배치되어, 상하 슬라이스가
  동일한 레이턴시로 액티베이션에 접근한다.
- **상하 대칭 슬라이스**: 각 슬라이스에는 32×1 GEMV 코어 2개(양
  슬라이스 합 4개)와 코어별 Constant Cache, L1 Cache가 포함된다.
  디코딩 단계에서 두 슬라이스가 병렬로 서로 다른 배치 / 헤드 또는
  multi-query를 처리할 수 있다. SFU는 단일 인스턴스로 두 슬라이스
  사이에 배치되어, 양쪽 GEMV↔SFU 직결 FIFO의 배선 길이를 대칭으로
  유지한다.
- **우측 시스톨릭 어레이**: 32×32 GEMM 코어 1개(cascade가 16행에서
  끊겨 32×16 서브체인 2개로 나뉨)가 프리필 단계의 대규모 GEMM을
  담당한다.
...
L2 Cache와 상·하 슬라이스 간 배선 길이를 대칭화하여, 두 슬라이스의
액티베이션 접근 레이턴시를 동일하게 유지한다. 이는 소프트웨어 측에서
...
KV260의 1,248개 DSP48E2 중 대부분이 우측 시스톨릭 어레이 영역에 배치되며,
GEMV 및 SFU 뱅크는 LUT/BRAM 기반 연산으로 DSP 사용을 최소화한다.
자세한 배치는 리소스 활용 섹션(추후 합성 결과 반영)에서 다룬다.
...
.. note::
   ...
   리소스 사용량은 generate 파라미터에 따라 변동된다.
