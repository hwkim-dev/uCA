...
``IN_length`` 클럭 동안 **매 클럭 1 개 원소씩 직렬 누적** 한 뒤 스칼라
합을 방출합니다. GEMV 출력처럼 32 원소가 병렬로 도착하는 경로는 GEMV 측
reduction tree 가 이미 스칼라까지 축약하므로, SFU 는 이후 단순한 합산
/ 정규화만 수행합니다. Softmax 의 denominator 가 대표적인 사용례.
...
접근할 수 있는 SFU 의 단일 인스턴스를 중앙 가까이 배치하는 방향을
가정한다. GEMV 출력이 **직결 FIFO** 로 SFU 에 들어오므로 GEMV 코어
